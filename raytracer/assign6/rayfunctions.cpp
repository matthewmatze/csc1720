...
/*
 * model_tptr model
 * object_tptr hitobj - object that was hit by the ray
 * drgb_t &pixel) - where to add intensity
 * */
void add_diffuse(model_tptr model, object_tptr hitObj, drgb_t &pixel)
{
   light_tptr tmp = (light_tptr)(model->lgts->start());
   //Iterate the lgts calling illumination function for each light
   while(tmp!=NULL){
      tmp->illumination(model->lgts, hitObj, pixel);
      tmp = (light_tptr)(model->lgts->get_next());
   }
